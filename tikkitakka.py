...
frame1 = Frame(root)
frame1.pack()
titleLabel = Label(frame1 , text="Tic Tac Toe" , font=("Arial" , 26) , bg="white" , width=16 )
titleLabel.grid(row=0 , column=0)

optionFrame = Frame(root , bg="white")
optionFrame.pack()
...
def play(event):
    ...
    if button["text"] == " ":
        if turn == "x" :
            board[clicked] = turn
            if checkForWin(turn):
                winningLabel = Label(frame1 , text=f"{turn} wins the game", bg="white", font=("Arial" , 26),width=16 )
        ...
        if checkForDraw():
            drawLabel = Label(frame1 , text=f"Game Draw" , bg="white", font=("Arial" , 26), width = 16)
